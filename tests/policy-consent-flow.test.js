...
test('개인정보처리방침 v1.0에만 동의한 기존 회원은 v1.2 재동의 대상 — 게이트가 뜨고 hydrate는 보류된다', async () => {
  const env = await boot({
    session: SESSION,
    // 코드의 현재 PRIVACY_VERSION(core.PRIVACY_VERSION)은 v1.2이므로, v1.0에만
  ...
  await flush(env);
  assert.equal(env.core.PRIVACY_VERSION, 'v1.2', '코드 상수가 v1.2인지 먼저 확인');
  assert.equal(gateOpen(env), true, 'v1.0 동의만 있으면 v1.2 재동의 게이트가 떠야 한다');
