...
test('개인정보처리방침 v1.0에만 동의한 기존 회원은 v1.3 재동의 대상 — 게이트가 뜨고 hydrate는 보류된다', async () => {
  const env = await boot({
    session: SESSION,
    // 코드의 현재 PRIVACY_VERSION(core.PRIVACY_VERSION)은 v1.3이므로, v1.0에만
  ...
  await flush(env);
  assert.equal(env.core.PRIVACY_VERSION, 'v1.3', '코드 상수가 v1.3인지 먼저 확인');
  assert.equal(gateOpen(env), true, 'v1.0 동의만 있으면 v1.3 재동의 게이트가 떠야 한다');
  assert.equal(hydrateStarted(env), false, '재동의 전에는 STEP/계획/마진 기록을 불러오지 않는다');
});

test('현재 공개 중인 v1.2에 동의한 회원도 v1.3 공개 후에는 재동의 대상 — 게이트가 뜨고 hydrate는 보류된다', async () => {
  const env = await boot({
    session: SESSION,
    consentRows: [{ user_id: USER.id, terms_version: TERMS_VERSION, privacy_version: 'v1.2', source: 'email_signup' }]
  });
  await flush(env);
  assert.equal(gateOpen(env), true, 'v1.2 동의만 있으면 v1.3 재동의 게이트가 떠야 한다');
