-- LaunchDesk 개인정보처리방침 v1.2 → v1.3 개정에 따른 submit_setup_inquiry
-- 동의 버전 갱신.
--
-- 배경: v1.3은 운영 현황 "광고 기록"의 Meta 하루 합계 자동 기록(adlog-meta.js)
-- 도입에 따라, Meta에서 가져와 광고 기록으로 저장하는 항목(하루 광고비 · Meta
-- 귀속 구매금액 · 구매 건수 · Meta 광고계정 ID)과 그 보유 · 삭제 시점을 방침
-- 2 · 3 · 4번에 반영한 개정이다. policy-consent-core.js의 PRIVACY_VERSION도
-- 'v1.2' → 'v1.3'으로 함께 올렸다. 시행일은 실제 게시일에 정한다.
--
-- 20260923170000_setup_inquiries_privacy_v1_2.sql이 이미 7개 인자 시그니처와
-- 버전 불일치 차단(p_expected_privacy_version, PRIVACY_VERSION_MISMATCH)을
-- 만들었다. 시그니처가 같으므로 이번에는 CREATE OR REPLACE로 서버 상수
-- (v_consent_version)만 'v1.3'으로 바꾼다 — 옛 시그니처 DROP이 필요 없고,
-- 인자 · SECURITY DEFINER · search_path · 검증 로직 · 플랜 가격 · INSERT 컬럼은
-- v1.2와 완전히 같다. 과거 신청 행의 동의 버전은 소급 수정하지 않는다.
--
-- 적용 시점: 개인정보처리방침 v1.3 게시와 함께(RPC 먼저 → 곧바로 웹). 이 파일을
-- 적용한 순간부터 아직 v1.2 화면을 보는 사용자의 세팅 대행 신청은
-- PRIVACY_VERSION_MISMATCH로 거부된다(setup.js가 "새로고침" 안내) — 잘못된
-- 버전이 기록되지는 않는다. 적용 후 setup_inquiry_consent_version_check.sql로
-- 상수가 v1.3인지 확인한다.
...
declare
  ...
  -- [개인정보처리방침 v1.3 개정] 서버 상수만 갱신한다 — 클라이언트는
  -- 여전히 이 값을 지정할 방법이 없다(저장에는 이 상수만 쓰인다).
  v_consent_version constant text := 'v1.3';
begin
