/* ============================================================================
   step02.js — STEP 02(#/start/setup) 화면 부가 동작 (2026-09 UI 재설계, 피그마 Step02.tsx 이식)
   ----------------------------------------------------------------------------
   저장·완료 판정은 건드리지 않는다. 그건 app.js 체크리스트 컨트롤러
   (.checklist-block[data-chapter="/start/setup"] · .cl-row input[type="checkbox"] 7개 ·
   change 이벤트 → setChapterDone / checklistAllChecked · .count 갱신)가 그대로 담당한다.
   여기서는 같은 체크박스의 checked 상태를 "읽기만" 해서
     · 진행 패널(필수 N / 7 · 진행 바 · 그룹별 개수 · 항목별 체크 · 준비 완료 표시)
     · 체크리스트 카드 안의 진행 바 · 완료 안내 · 행 강조(.done)
     · 하단 이전/다음 상태 문구
     · 가이드 탭 CTA → 오픈 체크리스트 탭 전환 + 맨 위로
     · 비회원 "로그인하고 저장하기" → 기존 로그인 모달
     · 미완료 상태에서 다음 STEP 이동 시 confirm 한 번(막지 않음)
   만 처리한다. 체크박스에 change 리스너를 하나 더 붙이지만 UI 갱신만 하고
   저장 경로(setChapterDone)는 호출하지 않는다 — 저장·토스트·GA4는 app.js에서 1회.
   ============================================================================ */
(function(){
  var root = document.getElementById('step02Root');
  if(!root) return;

  var byId = function(id){ return document.getElementById(id); };
  var block = root.querySelector('.checklist-block[data-chapter="/start/setup"]');
  ...
  var badge = byId('s2ProgressBadge');
  var fill = byId('s2ProgressFill');
  var itemsEl = byId('s2ProgressItems');
  var doneEl = byId('s2ProgressDone');
  var clFill = byId('s2ClFill');
  var clDone = byId('s2ClDone');
  var navStatus = byId('s2NavStatus');
  var navText = byId('s2NavText');
  var navCount = byId('s2NavCount');
  var nextLink = byId('s2Next');
  ...
  function render(){
    ...
    if(navStatus){
      ...
      if(navText) navText.textContent = complete ? 'STEP 02 준비 완료 — 다음 단계로 이동할 수 있어요.' : '체크 항목 ' + total + '개를 완료하면 STEP 02가 완료돼요.';
  ...
  document.addEventListener('click', function(e){
    var btn = e.target.closest('#view-start-setup .s2-tabbar .tab-btn');
    if(!btn) return;
    root.setAttribute('data-tab', btn.getAttribute('data-tab') === 'checklist' ? 'checklist' : 'guide');
  });

  // 가이드 하단 CTA → 오픈 체크리스트 탭 + STEP 제목(맨 위)으로. hash는 그대로, 체크 상태는 DOM에 그대로 남는다.
  var goBtn = byId('s2GoChecklist');
  if(goBtn) goBtn.addEventListener('click', function(){
    var tabBtn = root.querySelector('.s2-tabbar .tab-btn[data-tab="checklist"]');
    if(tabBtn) tabBtn.click();
    window.scrollTo(0, 0);
    var title = byId('s2ChecklistTitle');
  ...
  var loginBtn = byId('s2GuestLogin');
